...
describe("config schema", () => {
  ...
  describe("parseConfig", () => {
    it("should return defaults for undefined input", () => {
      ...
      expect(config.include).toHaveLength(14);
    ...
    it("should fallback to defaults for non-array include", () => {
      expect(parseConfig({ include: "string" }).include).toHaveLength(14);
      expect(parseConfig({ include: 123 }).include).toHaveLength(14);
    });

    it("should fallback to defaults for include with non-string items", () => {
      expect(parseConfig({ include: [123, 456] }).include).toHaveLength(14);
      expect(parseConfig({ include: ["valid", 123] }).include).toHaveLength(14);
    });
